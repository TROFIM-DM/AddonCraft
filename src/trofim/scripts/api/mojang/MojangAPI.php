...
{
    /**
     * Получить статус методов Mojang.
     *
    ...
    public static function getSessionProfile ($uuid) {
        if (self::isValidUUID($uuid)) {
            return self::fetchJson('https://sessionserver.mojang.com/session/minecraft/profile/'
                . self::minifyUUID($uuid));
        }
        return false;
    }
    ...
    public static function authenticate ($id, $password) {
        ...
        $httpClient->requestType = 'JSON';
        $post = $httpClient->post('https://authserver.mojang.com/authenticate', ['agent' => ['name' => 'Minecraft', 'version' => '1'], 'username' => $id, 'password' => $password]);
        return $post;
    ...
    private static function fetch ($url) {
        $stream = Stream::getContents($url);
        return empty($stream) ? false : $stream;
    }
